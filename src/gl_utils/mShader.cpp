...
mShader::mShader(std::string vs_path, std::string fs_path) {
    ...
    if (!vertexShaderStream || ! fragShaderStream) {
        std::cout << "Shader file is not exist!" << std::endl;
    }
...
mShader::mShader(std::string vs_path, std::string fs_path, std::string gs_path) {
    GLuint vertexShaderID = glCreateShader(GL_VERTEX_SHADER);
    GLuint fragShaderID = glCreateShader(GL_FRAGMENT_SHADER);
    ...
    std::ifstream geoShaderStream(gs_path, std::ios::in);

    if (!vertexShaderStream || !fragShaderStream || !geoShaderStream) {
        std::cout << "Shader file is not exist!" << std::endl;
    }
    ...
    if (!result) {
        std::vector<char> geoShaderErrorMsg(infoLogLength + 1);
        glGetShaderInfoLog(geoShaderID, infoLogLength + 1, NULL, &geoShaderErrorMsg[0]);
        printf("%s\n", &geoShaderErrorMsg[0]);
    }
